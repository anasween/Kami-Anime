...
class YumAuthController extends YumController {
    ...
    public function loginByUsername() {
        ...
        if ($user) {
            ...
            Yum::log(Yum::t('Non-existent user {username} tried to log in (Ip-Address: {ip})', array(
    ...
    public function logFailedLoginAttempts($user) {
        Yum::log(Yum::t('Failed login attempt for user {username} (Ip-Address: {ip})', array(
                    '{ip}' => Yii::app()->request->getUserHostAddress(),
                    '{username}' => $this->loginForm->username)), 'error');
        ...
        switch ($identity->errorCode) {
            ...
            case YumUserIdentity::ERROR_NONE:
                ...
                if ($user->failedloginattempts > 0) {
                    Yum::setFlash(Yum::t('Warning: there have been {count} failed login attempts', array(
    ...
    public function actionLogin($hybridauth = false) {
        ...
        if (isset($_POST['YumUserLogin'])) {
            ...
            if ($success !== false && $success instanceof YumUser) {
                ...
                Yum::log(Yum::t('User {username} successfully logged in by {login_type} (Ip: {ip})', array(
                ...
                Yii::app()->user->setState('yum-login-attempts', Yii::app()->user->getState('yum-login-attempts', 0) + 1);
            }
        }
