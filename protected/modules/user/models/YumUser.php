...
class YumUser extends YumActiveRecord {
        ...
        if (Yum::hasModule('profile') && $this->profile) {
            ...
            foreach (YumProfile::getProfileFields() as $column) {
                if ($this->profile->{$column}) {
                    $criteria->compare(
                            'profile.' . $column, $this->profile->{$column}, true);
                }
        ...
        if ($password) {
            ...
            if ($this->validate()) {
                $this->password = CPasswordHelper::hashPassword(
                                $password, Yum::module()->passwordHashCost);
                if (!$this->isNewRecord) {
    ...
    public function assignRole($role_title) {
        ...
        if ($role) {
            return Yii::app()->db->createCommand(sprintf(
                                    'insert into %s (user_id, role_id) values(%s, %s)', Yum::module('role')->userRoleTable, $this->id, $role->id))->execute();
        } else {
            return false;
        }
    }
            ...
            if ($user = $profile->user) {
                ...
                if ($user->activationKey == urldecode($key)) {
                    ...
                    if ($user->save(false, array('activationKey', 'status'))) {
                        ...
                        if (Yum::hasModule('message') && Yum::module('registration')->enableActivationConfirmation) {
    ...
    public function syncRoles($roles = null) {
        if (Yum::hasModule('role')) {
            ...
            $query = sprintf("delete from %s where user_id = %s", Yum::module('role')->userRoleTable, $this->id
            );
            $result = Yii::app()->db->createCommand($query)->execute();
            if ($roles) {
                foreach ($roles as $role) {
                    $query = sprintf("insert into %s (user_id, role_id) values(%s, %s)", Yum::module('role')->userRoleTable, $this->id, $role
                    );
                    $result = Yii::app()->db->createCommand($query)->execute();
                }
            }
