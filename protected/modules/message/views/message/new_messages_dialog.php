...
$this->beginWidget('zii.widgets.jui.CJuiDialog', array(
        ...
        'options'=>array(
            ...
            'modal' => 'false',
            'width' => '800px',
            'title' => Yum::t('You have {count} new Messages !', array('count' => count($messages))),
            'autoOpen'=>true,
        ),
    )
);
...
foreach($messages as $message) 
{
    if(is_object($message) && $message->from_user instanceof YumUser )
    {
            printf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>',
                CHtml::link($message->from_user->username, array('//profile/profile/view', 'id' => $message->from_user_id)),
